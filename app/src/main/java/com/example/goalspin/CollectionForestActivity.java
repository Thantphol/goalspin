...
public class CollectionForestActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_collection_forest);
            ...
            btnUse.setOnClickListener(v -> {
                ...
                try {
                    if (selectedItemType == 2) {
                        // Background
                        editor.putInt("CURRENT_BACKGROUND", R.drawable.hidden_forest);
                        editor.apply();
                        ...
                        // Pet
                        editor.putInt("CURRENT_PET", R.drawable.niffler);
                        editor.apply();
                        Toast.makeText(this, "Pet Equipped!", Toast.LENGTH_SHORT).show();

                    } else if (selectedItemType == 3) {
                        // Frame Theme
                        editor.putInt("CURRENT_HEADER_BG", R.drawable.header_forest);
                        editor.putInt("CURRENT_TASK_BG", R.drawable.forest_task);
                        editor.putInt("CURRENT_THEME_COLLECTION_FRAME", R.drawable.hiddenforest_frame);
                        editor.putInt("CURRENT_THEME_FRAME", R.drawable.forest_frame);
                        editor.putString("CURRENT_THEME_TITLE", "Warden of the Enchanted Grove");
                        editor.apply();

                        updateHeader();
                        updateCollectionFrame(); // *** เพิ่มบรรทัดนี้: อัปเดตกรอบทันที ***

                        Toast.makeText(this, "Forest Theme Equipped!", Toast.LENGTH_SHORT).show();
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    private void updateBackground() {
        try {
            ...
            View root = findViewById(R.id.forestRoot);
