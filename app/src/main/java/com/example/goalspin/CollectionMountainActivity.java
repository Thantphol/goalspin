...
public class CollectionMountainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_collection_mountain);
            ...
            btnUse.setOnClickListener(v -> {
                ...
                try {
                    if (selectedItemType == 2) {
                        // Background
                        editor.putInt("CURRENT_BACKGROUND", R.drawable.gothic_mountain);
                        editor.apply();
                        ...
                        // Pet
                        editor.putInt("CURRENT_PET", R.drawable.dragon);
                        editor.apply();
                        Toast.makeText(this, "Pet Equipped!", Toast.LENGTH_SHORT).show();

                    } else if (selectedItemType == 3) {
                        // Frame Theme
                        editor.putInt("CURRENT_HEADER_BG", R.drawable.header_mountain);
                        editor.putInt("CURRENT_TASK_BG", R.drawable.mountain_task);
                        editor.putInt("CURRENT_THEME_COLLECTION_FRAME", R.drawable.gothicmountain_frame);
                        editor.putInt("CURRENT_THEME_FRAME", R.drawable.mountain_frame);
                        editor.putString("CURRENT_THEME_TITLE", "Dragonblood Harbinger");
                        editor.apply();

                        updateHeader();
                        updateCollectionFrame(); // *** เพิ่มบรรทัดนี้: อัปเดตกรอบทันที ***

                        Toast.makeText(this, "Mountain Theme Equipped!", Toast.LENGTH_SHORT).show();
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    private void updateBackground() {
        try {
            ...
            View root = findViewById(R.id.mountainRoot);
