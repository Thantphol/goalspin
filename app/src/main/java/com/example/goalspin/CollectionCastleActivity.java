...
public class CollectionCastleActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_collection_castle);
            ...
            btnUse.setOnClickListener(v -> {
                ...
                try {
                    if (selectedItemType == 2) {
                        // Background
                        editor.putInt("CURRENT_BACKGROUND", R.drawable.floating_castle);
                        editor.apply();
                        ...
                        // Pet
                        editor.putInt("CURRENT_PET", R.drawable.owl);
                        editor.apply();
                        Toast.makeText(this, "Pet Equipped!", Toast.LENGTH_SHORT).show();

                    } else if (selectedItemType == 3) {
                        // Frame Theme
                        editor.putInt("CURRENT_HEADER_BG", R.drawable.header_castle);
                        editor.putInt("CURRENT_TASK_BG", R.drawable.castle_task);
                        editor.putInt("CURRENT_THEME_COLLECTION_FRAME", R.drawable.floatingcastle_frame);
                        editor.putInt("CURRENT_THEME_FRAME", R.drawable.castle_frame);
                        editor.putString("CURRENT_THEME_TITLE", "Celestial Citadel Keeper");
                        editor.apply();

                        updateHeader();
                        updateCollectionFrame(); // *** เพิ่มบรรทัดนี้: อัปเดตกรอบทันที ***

                        Toast.makeText(this, "Castle Theme Equipped!", Toast.LENGTH_SHORT).show();
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    private void updateBackground() {
        try {
            ...
            View root = findViewById(R.id.castleRoot);
